<template>
  <div class="screen">
    <div class="top">
      <div>
        <div class="title">Brainstormer</div>
        <div class="sub">Propose tes critères dans chaque catégorie · Visible par tous en temps réel</div>
      </div>
    ...
    <div class="board" :style="{ gridTemplateColumns: `repeat(${cols.length}, 1fr)` }">
      <div v-for="col in cols" :key="col.key" class="col"
        ...
        <input class="colin" :placeholder="`Ajouter un critère… (Entrée)`"
...
const cols  = computed(() => store.cols.length ? store.cols : RETRO_FORMATS['dod'].cols)
